...
public class McUtils {
	...
	private static final String MC_CACHE = "mcCache";
	...
	public static List<Site> getSiteList(){
		...
		List<Site> siteList = (List<Site>)CacheUtils.get(MC_CACHE, "siteList");
		if (siteList == null){
			Page<Site> page = new Page<Site>(1, -1);
			page = siteService.findPage(page, new Site());
			siteList = page.getList();
			CacheUtils.put(MC_CACHE, "siteList", siteList);
		}
		return siteList;
	}
	...
	public static List<Category> getMainNavList(String siteId){
		@SuppressWarnings("unchecked")
		List<Category> mainNavList = (List<Category>)CacheUtils.get(MC_CACHE, "mainNavList_"+siteId);
		if (mainNavList == null){
			...
			page = categoryService.findPage(page, category);
			mainNavList = page.getList();
			CacheUtils.put(MC_CACHE, "mainNavList_"+siteId, mainNavList);
		}
		return mainNavList;
	}
	...
	public static List<Article> getArticleList(String siteId, String categoryId, int number, String param){
		...
		category.setParentCodes(categoryId);
	...
	public static List<Link> getLinkList(String siteId, String categoryId, int number, String param){
		...
		page = linkService.findPage(page, link);
		return page.getList();
	}
	...
	public static Object getCache(String key) {
		return CacheUtils.get(MC_CACHE, key);
	}

	public static void putCache(String key, Object value) {
		CacheUtils.put(MC_CACHE, key, value);
	}

	public static void removeCache(String key) {
		CacheUtils.remove(MC_CACHE, key);
	}
    ...
    public static String getUrlDynamic(Article article) {
        ...
        str.append("/view-").append(article.getCategory().getId()).append("-").append(article.getId()).append(Global.getConfig("urlSuffix"));
        return str.toString();
    }
    ...
    public static String getUrlDynamic(Category category) {
        ...
        StringBuilder str = new StringBuilder();
        str.append(context.getContextPath()).append(Global.getFrontPath());
        str.append("/list-").append(category.getId()).append(Global.getConfig("urlSuffix"));
        return str.toString();
    }
    ...
    public static void addViewConfigAttribute(Model model, Category category){
        ...
        boolean goon = true;
        do{
            if(c.getParent() == null || c.getParent().getIsRoot()){
                goon = false;
            }
            categoryList.add(c);
            c = c.getParent();
